...
export default {
  ...
  methods: {
    ...
    submitbill() {
      ...
      for (let i = 0; i < this.goodslist.length; i++) {
        ...
        this.$api.user.goodlist(goodid).then(res => {
          ...
          let orderid = new Date().dataFormat('yyyyMMddhhmmss')
          console.log(JSON.stringify(payinfo) + payinfo.storeid+'orderid'+orderid);
    ...
    dateFormat(time) {
      ...
      // 拼接
      return orderid = year +month +day +hours +minutes +seconds
      console.log(orderid)
    }
  },
  created() {
