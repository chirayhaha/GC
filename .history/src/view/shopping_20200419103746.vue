    ...
    <div class="gwc">
      ...
      <div class="info">
        <!-- <div @click="selected(cart.good.id)">
          <el-checkbox @change="unselected(cart.good.id)" class="checkbox" v-model="cart.good.selected"></el-checkbox>
        </div>
        <div>
        ...
        <el-checkbox-group v-model="carinfo" @change="handleCheckedCitiesChange">
          <el-checkbox v-for="cart in carinfo" :label="cart" :key="cart" v-model="cart.good.selected">
            <div>
              <img class="img" />
              <!-- :src="goodslist.imgurl" -->
            </div>
            <div class="name">
              {{cart.good.goodname}}
              <div class="num">
...
export default {
  ...
  methods: {
    ...
    handleCheckAllChange(val) {
      this.carinfo.selected = val == 1 ? true : false;
      this.isIndeterminate = false;
    },
    handleCheckedCitiesChange(value) {
      let checkedCount = value.length;
      this.checkAll = checkedCount === this.carinfo.length;
      this.isIndeterminate =
        checkedCount > 0 && checkedCount < this.carinfo.length;
    },
    selected(goodid) {
      let userid = JSON.parse(getLocalStorage("userid"));
