  ...
  <div class="orderconfirm">
    ...
    <div class="middle">
      ...
      <div class="paypage" v-if="paying">
        <router-link  to="orderpage"><span>x</span></router-link>
        <br />
    ...
    submitbill() {
      ...
      for (let i = 0; i < this.goodslist.length; i++) {
        ...
        this.$api.user.goodlist(goodid).then(res => {
          ...
          console.log(
            JSON.stringify(payinfo) + uidlast + rannum + "or" + orderid
          );
          this.$api.user.createbill(payinfo).then(res => {
            this.$api.user.delcart().then(res=>{/* this.$message('删除成功') */});
          });
        });
      }
    },
...
a{text-decoration: none;color: rgb(189, 189, 146);}
.middle {
  flex: 16;
  height: 513px;
  overflow: hidden;
