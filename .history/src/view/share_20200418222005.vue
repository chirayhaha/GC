  ...
  <div class="share">
    ...
    >
      ...
      <el-form-item label="食材" prop="context" class="context">
        ...
        <el-tooltip class="item" effect="dark" content="输入后点击此方可成功输入" placement="top">
          <el-button><i class="el-icon-zoom-in" @click="searchgood(uploadForm.context)"></i></el-button>
        </el-tooltip>
        <el-tag v-for="tag in uploadForm.tags" :key="tag.name" closable>{{tag}}</el-tag>
        <div class="goodslist" v-if="show">
          <ul v-for="(info,index) in goodsinfo" :key="index" >
            <li @click="setbookgoods(info.id,info.goodname)">
              <span>{{info.goodname}}</span><span>￥{{info.goodprice}}</span>
            </li>
          </ul>
        </div>
      </el-form-item>
