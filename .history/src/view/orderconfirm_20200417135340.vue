  ...
  methods: {
    ...
    submitbill() {
      ...
      this.goodslist.forEach((item,index)=>{
          good.push(item[index])
      })
      
          let ordernum=[]
      ordernum.push(good.goodnum)
      let address = this.addresslist[this.no].province+this.addresslist[this.no].city
      +this.addresslist[this.no].area+this.addresslist[this.no].add
      let conphone = this.addresslist[this.no].conphone
      console.log(address +JSON.stringify(good)+ JSON.stringify(ordernum))
    }
  },
  created() {
    this.getorder();
