    ...
    submitbill() {
      ...
      for (let i = 0; i < this.goodslist.length; i++) {
        ...
        this.$api.user.goodlist(goodid).then(res => {
          ...
          let uidlast = userid.substr(7,10)
          let rannum = parseInt((Math.random()+100)*9)
          let orderid= uidlast+ this.dateFormat(new Date())//18
          console.log(JSON.stringify(payinfo)+uidlast+rannum+'or'+orderid);
